...
public class CommandEvaluator {
	...
	private boolean evaluate(Eval ast) throws Exception, PPException {
		...
		throw new UnsupportedOperationException(
				"Error evaluating expression " + ((ast != null) ? ast.ppl.getSource() : ""));
	}
	...
	private static class Eval {
		...
		public void warning(String message) {
			if (listener != null) {
				...
				listener.warning(message, ln, ast.getCharPositionInLine(), getText().length());
			}
		}
		...
		public Eval getNextSibling() {
			return new Eval(ppl, (PPLineAST) ast.getParent().getChild(ast.getIndex() + 1), listener);
		}
