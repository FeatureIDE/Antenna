...
public class Preprocessor {
	 ...
	 * 
	 */
	private Stack<Integer> m_statsStack;
	 ...
	 * @throws IOException
	 */
	public static void saveStrings(Vector<String> lines, OutputStream out, String encoding) throws IOException {
	 ...
	 * @throws IOException
	 */
	public static void loadStrings(Vector<String> lines, InputStream in, String encoding) throws IOException {
	 ...
	 * @throws PPException
	 */
	public boolean preprocess(Vector<String> lines, String encoding) throws IOException, PPException {
		try {
	 ...
	 * @throws PPException
	 */
	private boolean preprocessImpl(Vector<String> lines, String encoding) throws IOException, PPException {
			...
			try {
				if ((lp.getType() == PPLine.TYPE_VISIBLE) || (lp.getType() == PPLine.TYPE_HIDDEN)) {
					...
					if (ast.getType() == APPLexer.INCLUDE) { // include
						...
						Vector<String> includeLines = loadIncludedFile(includeLine, file, encoding);
	 ...
	 * @throws PPException
	 */
	private Vector<String> loadIncludedFile(int lineNum, String file, String encoding) throws PPException {
		File f;
	...
	String commentLine(PPLine lp) {
		if ((lp.getType() == PPLine.TYPE_VISIBLE)
				|| ((lp.prefixChar() != PPLine.HIDDEN_LINE_COMMENT_CHAR) && (lp.getType() != PPLine.TYPE_COMMAND))) {
	 ...
	 */
	private void handleCommand(Vector<String> lines, PPLine ppl, PPLineAST ast, CommandEvaluator evaluator,
			String encoding, boolean lastLine) throws Exception, PPException, UnsupportedEncodingException {
		...
		case APPLexer.CONDITION: {
			if (ppl.getLineNumber() != 0) {
				throw new PPException("//#condition is only allowed in the first line of the file", m_file,
						ppl.getLineNumber());
			}
			boolean r = evaluator.evaluate(ppl, ast, m_listener);
			handleCondition(r);
		...
		default:
			throw new PPException("Unexpected token " + APPParser.tokenNames[type] + " at \"" + ppl.getSource() + "\"",
	 ...
	 * @param lines
	 */
	private void handleExpand(PPLine ppl, Vector<String> lines) {
