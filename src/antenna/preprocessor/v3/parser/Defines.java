...
public class Defines {
	 ...
	 */
	private Hashtable<String, Define> m_defines;
			...
			if (returnAST.isNil()) {
				for (int i = 0; i < returnAST.getChildCount(); i++) {
					...
					for (PPLineAST t = (PPLineAST) returnAST.getChild(i); t != null
							&& t.getType() != APPLexer.EOF; t = (PPLineAST) t.getParent().getChild(t.getIndex() + 1)) {
				...
				for (PPLineAST node = returnAST; node != null && node.getType() != APPLexer.EOF; node = (PPLineAST) node
	...
	public void loadDefines(InputStream in) throws RecognitionException, IOException {
		...
		Hashtable<String, Define> backup = (Hashtable<String, Define>) m_defines.clone();
	...
	public void define(PPLineAST node) throws RecognitionException {
		...
		// FIXME
		if (node.getChild(0) != null && (node.getChild(0).getType() == APPLexer.ADD_IF_NEW /* add_if_new */
	...
	public String toString() {
		Enumeration<String> keys = m_defines.keys();
		Vector<String> v = new Vector<>(m_defines.size());
	...
	public Defines copy() {
		...
		copy.m_defines = (Hashtable<String, Define>) m_defines.clone();
		return copy;
	}
	 ...
	 */
	public void clear() {
		m_defines = new Hashtable<String, Define>();
	}
}
